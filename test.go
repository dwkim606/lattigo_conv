...
func testBootFast() {
	...
	// Generate rotations
	r_idx, _ := gen_extend_full(N/2, in_wid, 0, true, true)
	var rotations []int
	for k := range r_idx {
	...
	evaluator.Rotate(ciphertext2, params.Slots()/2, ciphertext2)
	evaluator.Rotate(ciphertext3, params.Slots()/2, ciphertext3)
	evaluator.DropLevel(ciphertext2, 2)
	evaluator.DropLevel(ciphertext3, 2)

	ciphertext := btp.BootstrappConv_StoC(ciphertext2, ciphertext3)
