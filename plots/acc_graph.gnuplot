...
axis_gap = 1000
f(x) = (x <= 4000) ? x : (x < 12000) ? NaN : (x - 8000 + axis_gap)
g(x) = (x <= 4000) ? x : (x < 4000 + axis_gap) ? NaN : (x + 8000 - axis_gap)
...
set arrow from 4000, graph 0 length graph -.01 angle 75 nohead lw 3 front
set arrow from 12000, graph 0 length graph  .01 angle 75 nohead lw 3 front
set arrow from 12000, graph 0 length graph -.01 angle 75 nohead lw 3 front
set arrow from screen 0.65, graph 0 length graph  .01 angle 75 nohead lw 3 front
set arrow from screen 0.65, graph 0 length graph -.01 angle 75 nohead lw 3 front
set arrow from screen 0.76, graph 0 length graph  .01 angle 75 nohead lw 3 front
set arrow from screen 0.76, graph 0 length graph -.01 angle 75 nohead lw 3 front

set arrow from 4000, graph 1 to 12000, graph 1 nohead lt 4000 lw 2 lc rgb "white" front
set arrow from 4000, graph 1 length graph  .01 angle 75 nohead lw 3 front
set arrow from 4000, graph 1 length graph -.01 angle 75 nohead lw 3 front
set arrow from 12000, graph 1 length graph  .01 angle 75 nohead lw 3 front
set arrow from 12000, graph 1 length graph -.01 angle 75 nohead lw 3 front
set arrow from screen 0.65, graph 1 length graph  .01 angle 75 nohead lw 3 front
set arrow from screen 0.65, graph 1 length graph -.01 angle 75 nohead lw 3 front
...
set arrow from 2258, 92.54 to 12041, 94.62 nohead dashtype 2 lt 1 lw 3 lc rgb '#0A1172'
...
plot 'acc.dat' using 1:2 with points ls 1 notitle, \
    ...
    'acc_she.dat' using 1:2 with points ls 1 notitle
